...
function formValidations()
{
	...
	if( selectedServices.options.length <= 0 ) { alert("Please select Service(s)"); return false; }
	
	if( periodTypeId == yearlyPeriodTypeName )
	{
		if( yearLB.selectedIndex < 0 ) { alert("Please select Year(s)"); return false; }
	}
	else
	{
		if( yearLB.selectedIndex < 0 ) { alert("Please select Year(s)"); return false; }
		if( periodLB.selectedIndex < 0 ) { alert("Please select Period(s)"); return false; }
	}
