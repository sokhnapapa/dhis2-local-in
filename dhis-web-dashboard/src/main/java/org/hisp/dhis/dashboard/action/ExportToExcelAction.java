    ...
    implements Action
{
        ...
        throws Exception
    {
        ...
        File outputReportFile = locationManager.getFileForWriting( UUID.randomUUID().toString() + ".xls", "db",
            "output" );

        /*
         * String outputReportPath = System.getProperty( "user.home" ) +
         * File.separator + "dhis" + File.separator + "ra" + File.separator +
    ...
    public void sortByAscending()
    {
        ...
        /*
         * for(int i=0; i < categories2.length-1 ; i++) { for(int j=0; j <
         * categories2.length-1-i; j++) { if(data2[0][j] > data2[0][j+1]) {
         * for(int k=0; k<series2.length; k++) { double temp1 = data2[k][j];
         * data2[k][j] = data2[k][j+1]; data2[k][j+1] = temp1; }
         * 
         * String temp2 = categories2[j]; categories2[j] = categories2[j+1];
