...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse; border-style: dotted" bordercolor="#111111" width="100%">
	...
	double data2[][]= new double[htForIndicatorValues.size()][];
	...
	while(keysForIndicatorValues.hasMoreElements())
	{
		...
		data2[count1] = new double[liForValues.size()];
		...
		{		
			...
			data2[count1][count2] = Math.round(iValue*Math.pow(10,2))/Math.pow(10,2);
...
<br>
<jsp:useBean id="myChart" scope="request" class='org.hisp.gtool.charts.DualAxisChart1' />

<jsp:setProperty name="myChart" property="series1" value="<%=series1%>"/>
<jsp:setProperty name="myChart" property="categories1" value="<%=categories1%>"/>
<jsp:setProperty name="myChart" property="data1" value="<%=data1%>"/>

<jsp:setProperty name="myChart" property="series2" value="<%=series2%>"/>
<jsp:setProperty name="myChart" property="categories2" value="<%=categories2%>"/>
